...
class no_grad:
    ...
    def __exit__(self, exc_type, exc, tb):
        ...
        # returning false ensures exceptions inside the block are not suppresed
        return False
...
class Tensor:
    """A differentiable array: wraps :class:`ap.Array` and optional autograd links.

    Parameters
    ----------
    data
        Scalars, sequences, NumPy/CuPy arrays, or :class:`ap.Array` instances.
    requires_grad : bool, default False
        If True, ``backward`` may populate ``self.grad`` when this tensor
        participates in tracked operations while graph building is enabled.
    grad_fn : Callable[[Any], None] or None
        Closure invoked during ``backward`` with ``grad_output``; responsible
        for propagating gradients to parents. ``None`` for leaves and
        non-differentiable tensors.
    grad_fn_name : str or None
        Debug label shown in ``repr`` (e.g. ``\"<AddBackward>\"``).
    device : str or None
        Passed through to :class:`ap.Array`.
    dtype : str or None
        Passed through to :class:`ap.Array`.

    Attributes
    ----------
    grad : ap.Array or numpy.ndarray or None
        Accumulated gradient of a scalar loss w.r.t. ``self.data``, same
        conceptual shape as ``self`` when populated.
    _parents : tuple[weakref.ref[Tensor], ...]
        Weak references to parent tensors for graph traversal (may contain
        fewer entries than arity if a parent was a non-Tensor constant).

    Notes
    -----
    Use :attr:`Tensor.build_graph_enabled` context or set
    :attr:`Tensor._build_graph` to disable graph construction for inference.
    """
    ...
    def __init__(
        ...
        self._data = ap.Array(data=data, device=device, dtype=dtype)
    ...
    @property
    def xp(self):
        """Backend module for elementwise ops: ``numpy`` or ``cupy`` (from ``self.data``)."""
        return self._data.xp

    @property
    def data(self):
        """Backing :class:`ap.Array`; assignable to replace buffer (creates new ``ap.Array``)."""
        return self._data
    ...
    @property
    def device(self):
        """Device string of the underlying array (e.g. ``\"cpu\"``, ``\"cuda:0\"``)."""
    ...
    @property
    def is_leaf(self):
        """True if ``requires_grad`` and this tensor was not created by an autograd op."""
        return self._is_leaf

    def __repr__(self):
        """Pretty-print values (truncated), grad metadata, and device on GPU."""
    ...
    def build_graph_enabled(cls):
        """Return whether new ops record ``grad_fn`` closures (class flag)."""
        return cls._build_graph

    @staticmethod
    def _check_broadcast(a, b):
        """Raise if ``a`` and ``b`` both require grad but have different ``ndim``.

        This is a conservative guard; full NumPy broadcasting allows more
        patterns when only one operand tracks gradients.

        Parameters
        ----------
        a, b : Tensor
            Binary operands after ``Tensor`` coercion of the RHS elsewhere.
        """
        if (len(a.shape) != len(b.shape)) and (a.requires_grad and b.requires_grad):
            raise ValueError(
                f"Incompatible Operation between {a.shape} and {b.shape}"
            )

    def _broadcasted_grad_accumulate(self, x_shape, x_grad):
        """Sum upstream gradients over axes where ``x`` was broadcast in the forward pass.

        **Why this exists**

        In reverse-mode AD, each op receives ``∂L/∂out`` (``grad_output``),
        shaped like the forward output ``out``. For elementwise binary ops,
        ``∂out/∂x`` is structurally simple, but *broadcasting* means several
        entries of ``out`` may depend on the *same* entry of ``x``.

        Formally, if forward broadcast replicates the value at ``x[i]`` into
        multiple output positions, those positions each add a term to
        ``∂L/∂x[i]`` via the chain rule. Those terms must be **added**
        together—equivalently, sum ``grad_output`` over the axes that were
        "expanded" from length-1 in ``x``'s shape.

        **What the implementation does**

        Let ``grad_shape = x_grad.shape`` (same as ``out.shape`` in typical
        elementwise ops). For each axis ``idx``:

        - If ``x_shape[idx] == 1`` and ``grad_shape[idx] != 1``, axis ``idx``
          was a broadcast dimension: ``x`` was conceptually *tiled* along this
          axis. We sum ``x_grad`` over ``idx`` so the post-reduction length
          is 1 again, matching ``x_shape[idx]``.
        - ``keepdims=True`` keeps length-1 axes so the result can be aligned
          with ``x_shape`` for in-place accumulation into ``x.grad``.

        Parameters
        ----------
        x_shape : tuple[int, ...]
            Shape of the tensor whose partial derivative we are computing
            (e.g. ``self.shape`` or ``other.shape`` before broadcasting).
        x_grad : array_like
            Gradient tensor shaped like the **forward output** (upstream
            ``∂L/∂out``), possibly on CPU as :class:`numpy.ndarray` depending
            on how the engine materializes ``grad_output``.

        Returns
        -------
        array_like
            Gradient reduced so each axis ``d`` matches ``x_shape[d]`` after
            summing over broadcast replication.

        Raises
        ------
        AssertionError
            If ``x_shape`` and ``x_grad.shape`` have different ranks (caller
            should ensure forwards used compatible broadcasting).

        Examples
        --------
        Forward: ``x`` has shape ``(1, 3)``, ``y`` has shape ``(4, 3)``.
        NumPy broadcasts ``x`` to ``(4, 3)``; ``out = x + y`` has shape
        ``(4, 3)``. Suppose ``∂L/∂out = G`` with shape ``(4, 3)``.

        For ``x`` (shape ``(1, 3)``), axis 0 was broadcast: ``x_shape[0]==1``
        but ``G.shape[0]==4``. This method computes ``G.sum(axis=0,
        keepdims=True)`` so the result has shape ``(1, 3)``, ready to add into
        ``x.grad``.

        For ``y`` (already ``(4, 3)``), no axis satisfies ``x_dim==1`` and
        ``grad_dim!=1`` simultaneously in a mismatched way; the list
        ``sum_axes`` is empty and ``x_grad`` is returned unchanged.

        Notes
        -----
        This matches the standard autograd treatment (same rule as PyTorch's
        ``sum_to`` / backward of ``torch.add`` under broadcast). The current
        implementation uses :func:`numpy.sum`; if ``x_grad`` is a CuPy array,
        callers may need a backend-aware reduction in the future.
        """
        grad_shape = x_grad.shape

        assert len(x_shape) == len(grad_shape), (
            f"grad rank {len(grad_shape)} must match x rank {len(x_shape)}; "
            "ensure forward shapes were broadcast-compatible."
        )
        ...
        if sum_axes:
            x_grad = np.sum(x_grad, axis=sum_axes, keepdims=True)
    ...
    def backward(self, grad = None, retain_graph=False):
        ...
        # for backprop. rev(topo_order) then backprop
    ...
    def __add__(self, val):
        ...
        Backward: dL/dA is input_grad -> (argument)
        """
        ...
        requires_grad = (
            self.requires_grad or val_requires_grad
    ...
    def __mul__(self, val):
        ...
        def _mul_backward(input_grad): # dL/dO
            """
            # input grad is dL/dO here
            O = self * val
            """
            if self.requires_grad:
        ...
        requires_grad = (self.requires_grad or val_requires_grad) and Tensor.build_graph_enabled()
        output = Tensor(
            output, requires_grad=requires_grad, grad_fn=_mul_backward if requires_grad else None, grad_fn_name="<MulBackward>" if requires_grad else None, device=self.device
        )
    ...
    def clamp(self, min_val=None, max_val=None):
        ...
        def _clamp_backward(input_grad):
            ...
            if self.requires_grad:
                ...
                mask = (self.data >= (min_val if min_val is not None else -np.inf)) & (self.data <= (max_val if max_val is not None else np.inf))
    ...
    def sqrt(self):
        ...
        if (self < 0).any():
            warnings.warn("sqrt operation received negative values, which will produce NaN", RuntimeWarning)
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, (list, tuple)):
            idx = tuple(
                ...
                if isinstance(i, (list, Tensor)) else i for i in idx
            )
    ...
    def transpose(self, dim1, dim2):
        ...
        Gradients will be (8, 12) we have to change to (12, 8)
        """
    ...
    def permute(self, *dims):
        ...
        (0, 1, 2)
        """
        # numpy / cupy -> swapaxes is equivalent to transpose in torch
        # numpy / cupy -> transpose is equivalent to permute in torch
    ...
    def reshape(self, *shape):
        ...
        """
        If we reshape it, we need to undo it for the backward pass
        Original:
        [1, 2, 3]
        [4, 5, 6]
    ...
    def flatten(self, start_dim=0, end_dim=-1):
        ...
        if start_dim == 0 and end_dim == self.data.ndim - 1:
            ...
            # example we have (a, b, c, d, e, f) & if we wanna flatten c,d,e this becomes (a, b, c*d*e, f)
            new_shape = (self.data.shape[:start_dim] + (int(np.prod(self.data.shape[start_dim: end_dim + 1])), ) + self.data.shape[end_dim+1: ])
    ...
    def unsqueeze(self, dim=0):
        ...
        def _unsqueeze_backward(input_grad):
            if self.requires_grad:
                ...
                grad = input_grad.sum(self.data, axis=dim)
    ...
    def squeeze(self, dim=None):
        ...
        """
        If no dimension is given remove all the 1 dimension's
        """
    ...
    def chunk(input, chunks, dim=0):
        ...
        for i in range(chunks):
            ...
            # each chunk will have it's own backward pass
    ...
    def sum(self, dim=None, keepdims=False):
        ...
        In PyTorch, dim=0 sums down the rows column-wise
    ...
    def mean(self, dim=None, keepdims=False):
        ...
        def _mean_backward(input_grad):
            ...
            if self.requires_grad:
                ...
                num_vals_averaged = np.prod([self.shape[d] for d in dim])
    ...
    def masked_fill(self, mask, value):
        ...
        def _masked_fill_backward(input_grad):
            if self.requires_grad:
                # Only pass gradients where mask == False
    ...
    def sort(self, dim=-1, descending=False):
        ...
        """
        # Forward: sort the data and get the indices
    ...
    def _add_parents(self, *parents):
        """Store weakrefs to differentiable parents (``None`` entries skipped).

        Parameters
        ----------
        *parents : Tensor or None
            Typically ``(self, other)`` from a binary op; constants are passed
            as ``None`` so they are not retained in the graph.
        """
    ...
    def item(self):
        if self.data.size != 1:
            raise ValueError("Only one element tensors can be converted to a python float")
        if "cuda" in self.device:
