...
def reshape_for_layernorm(x):
    ...
    """
    LayerNorm usually normalizes over the last dimension.
...
def auto_layernorm(input, weight, bias, eps=1e-5, *args):
    ...
    if weight is not None:
        scaled_shifted_x = norm_x * weight.reshape(1, -1) # typical unsqueeze
    else:
        scaled_shifted_x = norm_x
    
    if bias:
        scaled_shifted_x = scaled_shifted_x + bias.reshape(1, -1) # typical unsqueeze
    
    if reshaped_flag:
        scaled_shifted_x = scaled_shifted_x.reshape(*dims, embed_dim)
...
def manual_layernorm(input, weight, bias, eps=1e-5, *args):
    ...
    def _layernorm_backward(grad_output):
        ...
        if input.requires_grad:
            ...
            # sum up grads over the batch dim
            dx_hat = grad_output * weight_arr if weight_arr is not None else grad_output
...
def layernorm(input, weight, bias, eps=1e-5, auto=False, training=True, fused=False):
    ...
    if auto:
        ...
        return manual_layernorm(input, weight, bias, eps)