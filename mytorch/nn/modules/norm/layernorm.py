...
class LayerNorm(Module):
    ...
    def __init__(self, normalized_shape, weight=True, bias=True, eps=1e-5, auto=False, fused=False):
        ...
        self.bias = mytorch.ones((self.embed_dim,), requires_grad=True) if bias else None
    ...
    def forward(self, x):
        if not x.shape[-len(self.normalized_shape):] == self.normalized_shape:
            raise ValueError(f"Expected trailing dimensions to be {self.normalized_shape}, but got {x.shape[-len(self.normalized_shape):]}")
        
        original_shape = x.shape
        norm_ndim = len(self.normalized_shape)

        # Reshape: collapse normalized dims into last axis
        if norm_ndim > 1:
