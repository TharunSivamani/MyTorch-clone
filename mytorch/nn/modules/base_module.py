...
class Module:
    ...
    def parameters(self, memo=None):
        ...
        for param in self._parameters.values():
            if "cuda" in param.device:
                ptr = param.data.data.ptr # check Array class - Tensor.Array.cupy.ptr (decomposed)
            else:
                ptr = id(param.data)

            # check for - weight typing embedding layers
    ...
    def _named_parameters_no_dedup(self, prefix=""):
        ...
        for name, module in self._modules.items():
            sub_prefix = f"{prefix}{name}" if prefix else f"{name}."
            yield from module._named_parameters_no_dedup(sub_prefix) # recursive loop
        ...
        for name, buf in self._named_buffers_no_dedup(persistent_only=True):
            if name in state_dict:
                ...
                try:
                    ...
                    print(f"Failed to load {name}. Expected {buf.shape}, got {state_dict[name].shape}")
                    continue
